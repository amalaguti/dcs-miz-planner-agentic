...
_THEATRE_ALLOWED_TYPES: dict[str, frozenset[MissionType]] = {
    ...
    "Caucasus": frozenset({MissionType.FREE_FLIGHT, MissionType.CAP, MissionType.GROUND_ATTACK}),
}
...
def host_theatre_mission_refuse_nudge(spec: MissionSpec) -> str | None:
    ...
    TheChannel: all six. Normandy: all six. Caucasus: free_flight, CAP, and ground_attack.
    ...
    if spec.theatre == "Caucasus":
        return (
            "[Host] Caucasus invent is free_flight, CAP, or ground_attack at Batumi. "
            "CAP station 270°/40 km west over the Black Sea (not Manston 135/25, "
            "not Cherbourg 180/63). Ground attack 43°/110 km inland past Kutaisi "
            "(not CAP 270/40). Refuse intercept/escort/recon — "
            "emit free_flight, CAP, or ground_attack (Su-25T, Georgia blue, sunny_clear) "
    ...
    if spec.theatre == "Nevada":
        return (
            "[Host] Nevada invent is free_flight only at Nellis. "
            "Refuse intercept/cap/ground_attack/escort/recon — emit free_flight "
            "(Su-25T, USA blue, sunny_clear) or switch theatre to TheChannel. "
            "Do not copy channel_place or NeedsOarPoint geometry onto Nevada. "
            "Reply with a corrected Mission Spec JSON object ONLY (no markdown fences)."
        )
    if spec.theatre == "Falklands":
        return (
            "[Host] Falklands invent is free_flight only at Mount Pleasant. "
            "Refuse intercept/cap/ground_attack/escort/recon — emit free_flight "
            "(Su-25T, UK blue, sunny_clear) or switch theatre to TheChannel. "
...
def theatre_mission_refuse_chat_line(spec: MissionSpec) -> str:
    ...
    if spec.theatre == "Caucasus":
        return (
            "[Host] Caucasus intercept/escort/recon is not inventable — "
            "commander nudged toward Batumi free_flight, CAP, or ground_attack, "
            "or TheChannel. Draft NOT captured. Emit free_flight, CAP, or "
            "ground_attack at Batumi or switch theatre to TheChannel, then /accept."
        )
    if spec.theatre == "Syria":
        return (
    ...
    if spec.theatre == "Nevada":
        return (
            "[Host] Nevada combat (including CAP) is not inventable — "
            "commander nudged toward Nellis free_flight, or TheChannel. "
            "Draft NOT captured. Emit free_flight at Nellis or switch "
            "theatre to TheChannel, then /accept."
        )
    if spec.theatre == "Falklands":
        return (
            "[Host] Falklands combat (including CAP) is not inventable — "
            "commander nudged toward Mount Pleasant free_flight, or TheChannel. "
            "Draft NOT captured. Emit free_flight at Mount Pleasant or switch "
            "theatre to TheChannel, then /accept."
        )
    return (
...
def theatre_mission_refuse_accept_line(spec: MissionSpec) -> str:
    ...
    if spec.theatre == "Caucasus":
        return (
            "Caucasus intercept/escort/recon is not inventable. Draft NOT written. "
            "Emit free_flight, CAP, or ground_attack at Batumi or switch theatre to TheChannel."
        )
    if spec.theatre == "Syria":
        return (
    ...
    if spec.theatre == "Nevada":
        return (
            "Nevada combat (including CAP) is not inventable. Draft NOT written. "
            "Emit free_flight at Nellis or switch theatre to TheChannel."
        )
    if spec.theatre == "Falklands":
        return (
            "Falklands combat (including CAP) is not inventable. Draft NOT written. "
            "Emit free_flight at Mount Pleasant or switch theatre to TheChannel."
        )
    return (
...
def theatre_mission_refuse_planner_error(spec: MissionSpec) -> str:
    ...
    if spec.theatre == "Caucasus":
        return "Caucasus invent is free_flight, CAP, or ground_attack at Batumi; intercept/escort/recon are refused"
    if spec.theatre == "Syria":
        return "Syria invent is free_flight only; intercept/cap/GA/escort/recon are refused"
    if spec.theatre == "Nevada":
        return "Nevada invent is free_flight only; intercept/cap/GA/escort/recon are refused"
    if spec.theatre == "Falklands":
