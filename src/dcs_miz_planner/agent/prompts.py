...
- Theatre: any offerable theatre (known ∧ available ∧ planner_supported). Do not
  ...
  free_flight, CAP, ground_attack, or intercept (Batumi, Su-25T, sunny_clear, Georgia blue;
  CAP and intercept station 270°/40 km west over the Black Sea — not Manston 135/25, not
  Cherbourg 180/63, not Hawkinge; GA strike 43°/110 km inland past Kutaisi — not CAP 270/40).
  Syria invent is
  ...
  escort/recon on Caucasus and refuse
  ...
  Batumi FF/CAP/GA/intercept, Incirlik FF, Nellis FF, or Mount Pleasant FF)
...
- Caucasus geometry: copy kutaisi_inland_strike meta strike_bearing_deg /
  strike_distance_km (43° / 110 km inland past Kutaisi). CAP and intercept
  270° / 40 km is sea — do not put land trucks there. Do not copy french_coast 125/76 or
  Maupertus 180/133 onto Batumi. Call list_strike_targets(theatre=Caucasus) for
...
- Call get_mission_spec_schema(mission_type, theatre) before emitting Spec JSON and
  ...
  theatre=Caucasus for Batumi free_flight, CAP, ground_attack, or intercept; theatre=Syria for Incirlik free_flight;
    ...
    *,
    ...
    if "domain_unsupported_theatre" in err_l or "intercept_unsupported_theatre" in err_l:
        if theatre == "Falklands":
            ...
            geometry_hint = (
                "\n\nTheatre repair: for Caucasus intercept or CAP, use "
                "batumi_black_sea_cap 270° / 40 km (Black Sea, not Hawkinge/Dover, "
                "not Cherbourg 180/63). For land strike, use "
            ...
            allowed = {"free_flight", "cap", "ground_attack", "intercept"}
