...
INCIRLIK_INTERCEPT_EXAMPLE_SPEC = REPO_ROOT / "examples" / "incirlik_dawn_intercept.yaml"
...
def compile_incirlik_intercept(output_path: Path) -> Path:
...
INCIRLIK_RECON_EXAMPLE_SPEC = REPO_ROOT / "examples" / "incirlik_aleppo_recon.yaml"
INCIRLIK_RECON_MISSION_CONTRACTS = (
    "Su-25T",
    '["airdromeId"]=16',
    '["start_time"]=32400',
    "TakeOffParking",
...
def compile_incirlik_recon(output_path: Path) -> Path:
    spec = load_mission_spec(INCIRLIK_RECON_EXAMPLE_SPEC)
    return PyDCSCompiler(inventory=channel_available_inventory()).compile(
        spec, output_path, voice="raf"
    )
...
NEVADA_INTERCEPT_EXAMPLE_SPEC = REPO_ROOT / "examples" / "nellis_dawn_intercept.yaml"
...
def compile_nellis_intercept(output_path: Path) -> Path:
...
FALKLANDS_GA_EXAMPLE_SPEC = (
    REPO_ROOT / "examples" / "mount_pleasant_east_falkland_ground_attack.yaml"
)
FALKLANDS_GA_MISSION_CONTRACTS = (
    "Su-25T",
    '["airdromeId"]=2',
    '["start_time"]=32400',
    "TakeOffParking",
...
def compile_mount_pleasant_ground_attack(output_path: Path) -> Path:
    spec = load_mission_spec(FALKLANDS_GA_EXAMPLE_SPEC)
    return PyDCSCompiler(inventory=channel_available_inventory()).compile(
        spec, output_path, voice="raf"
    )
