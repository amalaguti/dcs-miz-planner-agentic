...
def test_host_spec_repair_nudge_syria_domain_uses_incirlik() -> None:
    from dcs_miz_planner.agent.prompts import host_spec_repair_nudge

    nudge = host_spec_repair_nudge(
        'Validation failed:\n[{"code": "domain_unsupported_theatre"}]',
        rejected_text='{"mission_type": "cap", "theatre": "Syria"}',
    )
    assert "Incirlik" in nudge
    assert "Su-25T" in nudge
    assert "Turkey" in nudge
    assert '"airfield": "Incirlik"' in nudge or '"airfield":"Incirlik"' in nudge
...
def test_host_spec_repair_nudge_nevada_mismatch_uses_creech() -> None:
    from dcs_miz_planner.agent.prompts import host_spec_repair_nudge
    ...
    nudge = host_spec_repair_nudge(
        ...
        rejected_text='{"mission_type": "ground_attack", "theatre": "Nevada", '
        '"player": {"airfield": "Nellis"}}',
    )
    assert "303" in nudge
    assert "85" in nudge
    assert "Creech" in nudge or "creech_range_strike" in nudge
    ...
    assert "350" not in nudge or "not CAP" in nudge or "not CAP/escort 350" in nudge


def test_host_spec_repair_nudge_nevada_recon_mismatch_uses_creech() -> None:
    from dcs_miz_planner.agent.prompts import host_spec_repair_nudge
    ...
    nudge = host_spec_repair_nudge(
        payload,
        mission_type="recon",
        rejected_text='{"mission_type": "recon", "theatre": "Nevada", '
        '"player": {"airfield": "Nellis"}}',
    )
    assert "303" in nudge
    assert "85" in nudge
...
def test_host_spec_repair_nudge_falklands_domain_uses_mount_pleasant() -> None:
    from dcs_miz_planner.agent.prompts import host_spec_repair_nudge

    nudge = host_spec_repair_nudge(
        'Validation failed:\n[{"code": "domain_unsupported_theatre"}]',
        rejected_text='{"mission_type": "cap", "theatre": "Falklands"}',
    )
...
def test_host_spec_repair_nudge_falklands_escort_uses_mount_pleasant() -> None:
    from dcs_miz_planner.agent.prompts import host_spec_repair_nudge

    nudge = host_spec_repair_nudge(
        'Validation failed:\n[{"code": "domain_unsupported_theatre"}]',
        rejected_text='{"mission_type": "escort", "theatre": "Falklands"}',
    )
    assert "Mount Pleasant" in nudge
    assert "150" in nudge
    assert "40" in nudge
    assert "escort" in nudge.lower()
    assert "120/55" not in nudge or "not Channel escort 120/55" in nudge
    assert '"airfield": "MountPleasant"' in nudge or '"airfield":"MountPleasant"' in nudge
    assert '"airfield": "Nellis"' not in nudge
    assert '"airfield":"Nellis"' not in nudge
    assert '"airfield": "Manston"' not in nudge
...
def test_host_spec_repair_nudge_falklands_recon_mismatch_uses_east_falkland() -> None:
    from dcs_miz_planner.agent.prompts import host_spec_repair_nudge
    ...
    nudge = host_spec_repair_nudge(
        payload,
        mission_type="recon",
        rejected_text='{"mission_type": "recon", "theatre": "Falklands", '
        '"player": {"airfield": "MountPleasant"}}',
    )
    assert "269" in nudge
    assert "21" in nudge
...
def test_host_spec_repair_nudge_syria_recon_mismatch_uses_aleppo() -> None:
    from dcs_miz_planner.agent.prompts import host_spec_repair_nudge
    ...
    nudge = host_spec_repair_nudge(
        payload,
        mission_type="recon",
        rejected_text='{"mission_type": "recon", "theatre": "Syria", '
        '"player": {"airfield": "Incirlik"}}',
    )
    assert "121" in nudge
    assert "200" in nudge
    ...
    assert "bearing_deg: 125" not in nudge
    assert "kutaisi_inland_strike" not in nudge
    assert "180" not in nudge or "not CAP" in nudge or "not CAP/escort 180" in nudge
