...
combat skeleton. When theatre is `Caucasus` and mission type is `free_flight`,
the derived example MUST follow the Batumi envelope (not Manston). When theatre
is `Caucasus` and mission type is `cap`, the derived example MUST follow the
Batumi Black Sea CAP envelope (not Manston 135/25, not Cherbourg 180/63).
When theatre is `Caucasus` and mission type is `ground_attack`, the derived
example MUST follow the Batumi inland ground-attack envelope (not Manston
125/76, not CAP 270/40). When theatre is `Caucasus` and mission type is
`intercept`, `escort`, or `recon`, the tool MUST NOT return a Channel or
...
Channel/Normandy/Caucasus template bundles. When mission type is `cap`, the
...
mission type is intercept/GA/escort/recon, the tool MUST NOT return a
...
#### Scenario: Syria intercept schema has no Manston skeleton
- **WHEN** a caller requests an intercept schema with theatre `Syria`
