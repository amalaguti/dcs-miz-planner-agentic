...
preferred_motion static, and preferred_ai_preset convoy_transit (soft AI, not
aaa_alert).
...
and `biggin_hill_home` with per-home cap/strike/escort geometry and
`max_flight_size`. Geometry MUST NOT copy Manston 135/25 or 125/76.
...
Mission options SHALL expose `channel_place` ids `chailey_home`, `tangmere_home`,
and `ford_af_home` with per-home geometry. Tangmere MUST publish
`max_flight_size` 3.
...
### Requirement: Recon is a narrative_pack mission type
Planning-options `mission_behaviour` id `narrative_pack` MUST list `recon` among
supported mission_types.

#### Scenario: narrative_pack includes recon
- **WHEN** catalog/registry loads narrative_pack
- **THEN** meta mission_types MUST include recon
