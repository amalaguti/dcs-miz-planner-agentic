...
nudges for `motion_domain_mismatch` or `strike_domain_mismatch` MUST use the
inferred theatre: TheChannel (or unspecified) SHALL mention Channel place
recipes (french-coast ~125°/76 km). Caucasus SHALL mention
`kutaisi_inland_strike` 43°/110 km and MUST NOT present french_coast 125/76 as
the template to copy. Normandy SHALL mention `maupertus_inland_strike`
180°/133 km and MUST NOT present french_coast 125/76 as the template to copy.
...
Invent/chat SHALL allow all six mission types when the bound theatre is
`Caucasus` (home `Batumi`, player `SpitfireLFMkIX`, `sunny_clear`, Georgia blue;
...
#### Scenario: Caucasus escort invent allowed
...
Invent/chat SHALL allow all six mission types when the bound theatre is
`Syria` (home `Incirlik`, player `SpitfireLFMkIX`, `sunny_clear`, Turkey blue;
CAP/intercept/escort station 180° / 40 km south over the Gulf of Iskenderun;
GA/recon AOI 121° / 200 km inland past Aleppo — not Manston 125/76, not
Cherbourg 180/63, not escort 120/55, not CAP 180/40 for land observe). Repair
MUST NOT copy Channel, Normandy, or Caucasus `channel_place` geometry onto
Syria.
...
the theatre home. Su-25T MAY remain on enemies or escort package when the ask
is Cold War or unspecified modern. Kola MUST remain free_flight only.
...
`FW-190A8` with country `ThirdReich` rather than defaulting Su-25T. It MUST
NOT invent MiG-15 or F-86 ids. Kola combat remains refused.
...
dynamics, and/or motion). Vague unpredictable / dice / different-each-load
asks MUST emit `dynamics.mode` `live`; F10 / I-choose asks MUST emit `choose`;
both MUST emit `hybrid`. Ground-attack or recon asks for moving convoy /
under way / patrol MUST set `targets[].motion` to `patrol` or `path`. The host
MUST NOT enable `narrative` together with `dynamics`. The host MUST NOT treat
CLI randomize as invent authoring.
