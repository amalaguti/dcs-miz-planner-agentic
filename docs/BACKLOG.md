...
| 17a | `weather-presets-expand` | Expand Channel Spec weather beyond the trio: curated **named patterns** (cloud gallery + wind/fog/turb recipes), pilot-facing brief text, SoT parity (`weather_sot`). Prefer recipes over raw ME knobs. **Research first:** R10 (ME/PyDCS/templates) + real Channel meteo → pattern cards; optional ME weather-template `.lua` as authoring aid only | `idea` (blocked on R10 meteo+templates notes; after M5) |
...
| R10 | `research-me-mission-content` | **ME content-depth pass** (in-editor + PyDCS + optional meteo): (1) **Weather** — full static panel vs our trio; ME **weather templates** (`Saved Games\DCS\MissionEditor\weather\dynamic\*.lua` = weather table slice); PyDCS `CloudPreset` / `dcs.cloud_presets` (30 ids; clamp base); wind/fog/turb/QNH; defer dynamic cyclones; save Channel recipe templates in ME as references. (2) Real **Channel meteo / climatology** → 5–8 pattern cards (not live METAR invent). (3) **Static objects** / other placement knobs. Notes under `research/` (e.g. `weather.md`); promote `#17a` / `#17b`. Complements R9 | `idea` (2026-08-06; ME browse + weather.md; templates/PyDCS pass started) |
...
- **ME mission-content depth** — research **R10**: weather panel / ME weather
  templates (`MissionEditor\weather\dynamic`) / PyDCS `CloudPreset` / Channel meteo
  pattern cards / statics; promote `#17a` / `#17b` after notes (`research/weather.md`).
