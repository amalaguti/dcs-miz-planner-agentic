...
| 30g | `qag-research-fixtures` | Index gitignored `research/` QAG HTML for offline `research_guidance` (thin YAML index only; pages stay local) | `done` (2026-08-16; pytest/API) |
...
**Next promote / in proposal:** Falklands intercept (`falklands-intercept`).
After `falklands-places`.
...
| F2d | `caucasus-places` | Caucasus Stage C: Batumi places + Black Sea CAP (270°/40 km); invent FF+CAP; intercept/GA/escort/recon still refuse | `done` (CLI/API 2026-08-16; ME Instant Action do-soon) |
...
promote time (`<theatre>-airfields`, `<theatre>-places`, …). Next promote: Falklands intercept (`falklands-intercept`). Refresh
...
| Audit owned maps (Normandy, Syria, Marianas, …) for multi-theatre expand | **Research R11 done**; product expand → **M7** (Slice 0 + 0b + F1–F5 Stage A done; unbound maps discovered-only) |
