...
**Next promote / in proposal:** Full-catalog campaign **M7** (Slice F1
`normandy-airfields-places`) when started from Agents Window
...
**Next promote / in proposal:** `normandy-airfields-places` (F1). Trigger from
...
- **Normandy / multi-theatre** — **M7** full-catalog campaign (`normandy-airfields-places` next after Slice 0b). R11 audit done; campaigns remain inspiration, not Spec import.
...
| Audit owned maps (Normandy, Syria, Marianas, …) for multi-theatre expand | **Research R11 done**; product expand → **M7** (Slice 0 + 0b done; `normandy-airfields-places` next) |
