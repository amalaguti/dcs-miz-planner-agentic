...
| 30e | `mission-designer-catalog` | Catalog shelves for mission-designer co-authoring: dynamics modes (live/choose/hybrid/fixed), strike target classes ↔ payload/domain, curated Channel places; agent prompts query catalog then recommend | `building` (apply in progress; `#30f` consumes shelves) |
| 30f | `mission-dynamics-pack` | Spec `dynamics` expand pack (pools + modes) emitting set_flag_random / F10 / activate_group — follows `#30e` palette | `idea` (waits on `#30e` shelves) |
