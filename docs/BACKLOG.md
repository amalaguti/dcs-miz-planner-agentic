...
**Next promote / in proposal:** Full-catalog campaign **M7** (Slice 0b
`theatre-agnostic-planning`) when started from Agents Window
...
**Next promote / in proposal:** `theatre-agnostic-planning` (Slice 0b). Trigger from
...
- **Normandy / multi-theatre** — **M7** full-catalog campaign (`theatre-agnostic-planning` next after Slice 0). R11 audit done; campaigns remain inspiration, not Spec import.
...
| Audit owned maps (Normandy, Syria, Marianas, …) for multi-theatre expand | **Research R11 done**; product expand → **M7** (Slice 0 done; `theatre-agnostic-planning` next) |
